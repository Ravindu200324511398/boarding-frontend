        ...
        <div className="row g-3 mb-4">
          {[0,1,2].map(idx=>(
            <div key={idx} className="col-md-4">
              <div style={{ background:'#fff', borderRadius:16, border:selected[idx]?'2px solid #2563eb':'2px dashed #cbd5e1', overflow:'hidden', boxShadow:'0 2px 12px rgba(15,23,42,0.06)', position:'relative' }}>
                {selected[idx] ? (
                  <div>
                    ...
                    <div style={{ padding:'0.9rem 1rem' }}>
                      ...
                      <div style={{ fontFamily:'var(--font-heading)', fontSize:'1.2rem', fontWeight:800, color:'#2563eb' }}>LKR {selected[idx].price.toLocaleString()}<span style={{ fontSize:'0.75rem', color:'#94a3b8', fontWeight:400 }}>/mo</span></div>
                    </div>
                  </div>
                ) : (
                  <div style={{ padding:'1.5rem 1rem' }}>
                    ...
                    <div style={{ position:'relative' }}>
                      ...
                      {dropdownOpen[idx]&&(
                        <div style={{ position:'absolute', top:'110%', left:0, right:0, background:'#fff', borderRadius:12, border:'1px solid #e2e8f0', boxShadow:'0 8px 32px rgba(15,23,42,0.12)', zIndex:100, overflow:'hidden', maxHeight:240, overflowY:'auto' }}>
            ...
            <CompareRow label="💰 Monthly Rent">
              {selected.filter(Boolean).map(b=>(
                <CellValue key={b._id} highlight={b.price===minPrice}>
                  <div style={{ fontFamily:'var(--font-heading)', fontSize:'1.4rem', fontWeight:800, color:b.price===minPrice?'#059669':'#0f172a' }}>LKR {b.price.toLocaleString()}</div>
