...
import {
  ...
  FiCalendar, FiMapPin, FiEye, FiCheckCircle, FiAlertCircle
} from 'react-icons/fi';
...
const UserAvatar = ({ user, size = 80 }) => {
  ...
  return (
    <div style={{
      ...
      border: '3px solid rgba(255,255,255,0.5)', overflow: 'hidden', flexShrink: 0,
      ...
      {avatarUrl ? (
        ...
        <span style={{ fontSize: size * 0.3, fontWeight: 800, color: '#fff', fontFamily: 'var(--font-heading)' }}>
          {initials}
        </span>
      )}
    </div>
...
const AdminUserDetail = () => {
  ...
  const handleDeleteBoarding = async (boardingId, title) => {
    ...
    try {
      ...
      setMsg({ type: 'success', text: `Listing "${title}" deleted.` });
      setTimeout(() => setMsg({ type: '', text: '' }), 3000);
    } catch (err) {
      setMsg({ type: 'error', text: err.response?.data?.message || 'Delete failed' });
    }
  ...
  return (
    <div style={{ background: '#f8fafc', minHeight: '100vh' }}>
      ...
      {/* ── Banner ── */}
      <div style={{ background: 'linear-gradient(135deg, #1e3a8a, #2563eb, #7c3aed)', padding: '2.5rem 0 4rem' }}>
        <div className="container" style={{ maxWidth: 900 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: '1.5rem', flexWrap: 'wrap' }}>
            ...
            <div style={{ color: '#fff', flex: 1 }}>
              <h1 style={{ fontFamily: 'var(--font-heading)', fontSize: '1.8rem', fontWeight: 800, margin: 0, marginBottom: '0.2rem' }}>
                {user.name}
              </h1>
        ...
        <div className="row g-3">
          ...
          <div className="col-md-4">
            <div style={{ background: '#fff', borderRadius: 16, boxShadow: '0 4px 20px rgba(15,23,42,0.08)', overflow: 'hidden' }}>
              ...
              <div style={{ background: 'linear-gradient(135deg, #1e3a8a, #2563eb)', padding: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.8rem' }}>
          ...
          <div className="col-md-8">
            <div style={{ background: '#fff', borderRadius: 16, boxShadow: '0 4px 20px rgba(15,23,42,0.08)', overflow: 'hidden' }}>
              <div style={{ padding: '1.2rem 1.5rem', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <h4 style={{ fontFamily: 'var(--font-heading)', fontWeight: 800, color: '#0f172a', margin: 0, fontSize: '1rem' }}>
                    Boarding Listings
                  </h4>
              ...
              {boardings.length === 0 ? (
                ...
                <div style={{ padding: '1rem 1.2rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  {boardings.map(b => {
                    ...
                    return (
                      <div key={b._id} style={{ display: 'flex', gap: '0.9rem', padding: '0.9rem 1rem', borderRadius: 12, border: '1px solid #e2e8f0', background: '#fafbfc', alignItems: 'center', transition: 'box-shadow 0.15s' }}
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontWeight: 700, fontSize: '0.9rem', color: '#0f172a', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginBottom: '0.2rem' }}>
                            {b.title}
                          </div>
                          <div style={{ fontSize: '0.78rem', color: '#64748b', display: 'flex', alignItems: 'center', gap: '0.35rem', marginBottom: '0.2rem' }}>
                            <FiMapPin size={10} /> {b.location}
                          </div>
                        ...
                        <div style={{ display: 'flex', gap: '0.4rem', flexShrink: 0 }}>
                          <Link to={`/boarding/${b._id}`} target="_blank">
                            <button title="View listing" style={{ background: '#dbeafe', color: '#2563eb', border: 'none', borderRadius: 8, padding: '0.45rem 0.75rem', cursor: 'pointer', fontSize: '0.78rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                              <FiEye size={12} /> View
                            </button>
                          </Link>
                          <button onClick={() => handleDeleteBoarding(b._id, b.title)} title="Delete listing"
