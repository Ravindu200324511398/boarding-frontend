...
import { FiUsers, FiHome, FiTrendingUp, FiDollarSign, FiEye, FiTrash2, FiShield } from 'react-icons/fi';
    ...
    <div>
      <div style={{ background:'linear-gradient(135deg, #1a1a2e 0%, #16213e 100%)', padding:'2.5rem 0', marginBottom:'2rem' }}>
        <div className="container">
          ...
          <p style={{ color:'rgba(255,255,255,0.6)', margin:0, fontSize:'0.95rem' }}>Overview of your Boarding Finder platform</p>
        </div>
      </div>
            ...
            <div style={{ background:'#fff', borderRadius:16, padding:'1.5rem', boxShadow:'0 4px 24px rgba(15,23,42,0.08)', border:'1px solid #e2e8f0' }}>
              ...
              <div style={{ overflowX:'auto' }}>
                <table style={{ width:'100%', borderCollapse:'collapse' }}>
                  ...
                  <tbody>
                    {stats.recentBoardings.map((b, i) => (
                      <tr key={b._id} style={{ borderBottom:'1px solid #f8fafc', transition:'background 0.15s' }}
                        onMouseEnter={e => e.currentTarget.style.background='#f8fafc'}
                        onMouseLeave={e => e.currentTarget.style.background='transparent'}>
                        <td style={{ padding:'0.8rem', fontSize:'0.875rem', fontWeight:600, color:'#0f172a', maxWidth:180, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{b.title}</td>
              ...
              <div className="row g-3">
                {stats.recentUsers.map(u => (
                  <div key={u._id} className="col-12 col-md-6 col-lg-4">
                    <div style={{ display:'flex', alignItems:'center', gap:'0.8rem', padding:'0.9rem', borderRadius:12, border:'1px solid #e2e8f0', background:'#f8fafc' }}>
                      <div style={{ width:40, height:40, borderRadius:'50%', background:'linear-gradient(135deg, #2563eb, #7c3aed)', display:'flex', alignItems:'center', justifyContent:'center', color:'#fff', fontWeight:700, fontSize:'1rem', flexShrink:0 }}>
                        {u.name.charAt(0).toUpperCase()}
                      </div>
                      <div style={{ minWidth:0 }}>
                        <div style={{ fontWeight:600, fontSize:'0.875rem', color:'#0f172a', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{u.name}</div>
                        <div style={{ fontSize:'0.78rem', color:'#94a3b8', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{u.email}</div>
                      </div>
                      <div style={{ marginLeft:'auto', flexShrink:0 }}>
                        <span style={{ background: u.isAdmin ? '#ede9fe' : '#f0fdf4', color: u.isAdmin ? '#7c3aed' : '#059669', padding:'0.2rem 0.6rem', borderRadius:20, fontSize:'0.72rem', fontWeight:600 }}>
